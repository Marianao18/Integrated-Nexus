...
const AdminSolicitudes = () => {
    ...
    useEffect(() => {
        const cargarDatos = async () => {
            try {
                const res = await axios.get(`${process.env.REACT_APP_API_URL}/admin/solicitudes/`, {
        ...
        if (window.confirm("¿Deseas convertir a este postulante en docente oficial?")) {
            try {
                const res = await axios.post(
                    `${process.env.REACT_APP_API_URL}/admin/aprobar-docente/${id}/`,
                    {},
                    {
                        headers: {
    ...
    const rechazar = async (id) => {
        if (window.confirm("¿Estás seguro de rechazar esta solicitud? Se enviará un correo de notificación.")) {
            try {
                ...
                const res = await axios.post(`${process.env.REACT_APP_API_URL}/admin/rechazar-docente/${id}/`, {}, {
                    headers: { Authorization: `Bearer ${localStorage.getItem('access_token')}` }
                });
